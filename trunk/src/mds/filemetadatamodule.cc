...
void FileMetaDataModule::createFile (uint32_t clientId, string path, uint64_t fileSize, uint32_t fileId)
{
	BSONObj insertObject = BSON ("id" << fileId << "path" << path << "fileSize" << (long long int)fileSize << "clientId" << clientId);
